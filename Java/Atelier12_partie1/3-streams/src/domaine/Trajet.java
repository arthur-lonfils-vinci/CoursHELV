...
public class Trajet implements Iterable<Caisse>{
    ...
    public Map<LocalDate, Long> nombreDeCaissesParDateLimite() {
        return caisses.stream().filter(caisse -> caisse.getDateLimite().isAfter(LocalDate.now())).collect(Collectors.groupingBy(caisse -> caisse.getDateLimite(), Collectors.counting()));
    }
}
