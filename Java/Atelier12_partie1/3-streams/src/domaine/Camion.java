...
public class Camion  {
    ...
    public List<Trajet> trouverTrajetsAvecPlaceRestante(){
        return trajets.stream().filter(trajet -> trajet.nbCaisses() != nbMaxCaisses).sorted(Comparator.comparing(Trajet::getVilleDepart).thenComparing(Trajet::getVilleArrivee)).collect(Collectors.toList());
    }
    ...
    public LocalDate trouverDateTrajet(String villeDepart, String villeDArrivee){
        return trajets.stream().filter(trajet -> trajet.getDate().isAfter(LocalDate.now())).filter(trajet -> trajet.getVilleDepart().equals(villeDepart) && trajet.getVilleArrivee().equals(villeDArrivee)).map(Trajet::getDate).sorted().findFirst().orElse(null);
    }
