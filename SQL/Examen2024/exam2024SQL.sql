...
CREATE
    OR REPLACE FUNCTION examen.formationInscription(_participant INTEGER, _formation INTEGER)
    RETURNS INTEGER AS
$$
DECLARE
...
BEGIN
    ...
    VALUES (_formation, _participant);
    SELECT count(DISTINCT i.participant)
    INTO nb_participants
...
CREATE
    OR REPLACE FUNCTION examen.checkInscription()
    RETURNS TRIGGER AS
$$
DECLARE
    max_niveau_part INTEGER;
    niveau_form
                    INTEGER;
    nbr_participant
                    INTEGER;
BEGIN
    IF
        (SELECT 1
         FROM examen.formations f
         WHERE f.id_formation = NEW.formation
           AND f.inscription_cloturee IS TRUE)
    THEN
        RAISE EXCEPTION 'EXCEPTION: inscription clôturée';
    END IF;
    ...
    IF
        (SELECT 1
         FROM examen.formations f
         WHERE f.id_formation = NEW.formation
           AND f.fdate < current_date)
    THEN
        RAISE EXCEPTION 'EXCEPTION: Date passée';
    END IF;
    ...
    WHERE f.id_formation = NEW.formation
    INTO niveau_form;
    SELECT COALESCE((SELECT f.niveau
                     FROM examen.formations f,
                          examen.inscriptions i
                     WHERE i.formation = f.id_formation
                       AND i.participant = NEW.participant
                     ORDER BY f.niveau DESC
                     LIMIT 1), NULL)
    INTO max_niveau_part;

    IF
        (niveau_form >= 2 AND (max_niveau_part IS NULL OR max_niveau_part + 1 < niveau_form))
    THEN
        RAISE EXCEPTION 'EXCEPTION: Niveau trop bas';
    END IF;

    SELECT COUNT(i.participant)
    FROM examen.inscriptions i
    WHERE i.formation = NEW.formation
    INTO nbr_participant;
    IF
        (nbr_participant >= (SELECT f.max_participants FROM examen.formations f WHERE f.id_formation = NEW.formation))
    THEN
        UPDATE examen.formations
        SET inscription_cloturee = TRUE
        WHERE examen.formations.id_formation = NEW.formation;
    END IF;
...
CREATE
    OR REPLACE TRIGGER triggerInscription
    BEFORE INSERT
    ON examen.inscriptions
    FOR EACH ROW
EXECUTE PROCEDURE examen.checkInscription();
...
VALUES (1, CURRENT_DATE + INTERVAL '10 days', 10),
...
UPDATE examen.formations
SET fdate = CURRENT_DATE - INTERVAL '5 days'
WHERE id_formation = 2;
