...
public class Graph {
	...
	public void bfs(Station depart) {
		...
		while (!queue.isEmpty()) {
			...
			toReturn.append(current.getNom()).append(' ');

			for (Troncon troncon : listeAdjacence) {
				if (troncon.getDepart().equals(current) && !visited.contains(troncon.getArrivee())){
					queue.offer(troncon.getArrivee());
					visited.add(troncon.getArrivee());
				}
			}
		}
	...
	public void dfs(Station depart) {
		...
		while(!queue.isEmpty()) {
			Station current = queue.pollFirst();
			toReturn.append(current.getNom()).append(' ');

			for (Troncon troncon : listeAdjacence) {
				if (troncon.getDepart().equals(current) && !visited.contains(troncon.getArrivee())) {
					queue.addFirst(troncon.getArrivee());
					visited.add(troncon.getArrivee());
				}
			}
		}
